...
If you need to use an ACL, please use the following command with the -a tag. Always specify root (/) with a *, as the command will not work otherwise (the exact reason for this is unclear):

The tag now with create the Unix Epoch time in hexadecimal value.
...
.. code-block:: bash
    ...
    $ python cms_edgeauth.py -k [key] -s now -w [time_in_seconds] -a "/*"
